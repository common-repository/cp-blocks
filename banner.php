	...
	function codepeople_add_promote_banner( $wp_admin_bar ) {
		...
		if ( isset( $_POST['codepeople_promote_banner_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['codepeople_promote_banner_nonce'] ) ), __FILE__ ) ) {
			if ( ! empty( $_POST['codepeople_promote_banner_plugin'] ) ) {
				$_promote_banner_plugin = sanitize_text_field( wp_unslash( $_POST['codepeople_promote_banner_plugin'] ) );

				if ( ! empty( $codepeople_promote_banner_plugins[ $_promote_banner_plugin ] ) ) {
					set_transient( 'codepeople_promote_banner_' . $_promote_banner_plugin, -1, 0 );
					if (
						! empty( $_POST['codepeople_promote_banner_action'] ) &&
						'set-review' == $_POST['codepeople_promote_banner_action'] &&
						! empty( $codepeople_promote_banner_plugins[ $_promote_banner_plugin ]['plugin_url'] )
					) {
						print '<script>document.location.href="' . str_replace( '&amp;', '&', esc_js( $codepeople_promote_banner_plugins[ $_promote_banner_plugin ]['plugin_url'] ) ) . '";</script>';
					}
				}
			}
		}
